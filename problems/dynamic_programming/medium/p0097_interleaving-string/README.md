...
**Link:** [LeetCode](https://leetcode.com/problems/interleaving-string/)
...
dp[i][j] represents whether s3[0:i+j] can be formed by interleaving s1[0:i] and s2[0:j]. At each position in s3, we must take a character from either s1 or s2, so dp[i][j] = (dp[i-1][j] and s1[i-1] == s3[i+j-1]) or (dp[i][j-1] and s2[j-1] == s3[i+j-1]).
...
1. Forgetting the length check at the beginning
